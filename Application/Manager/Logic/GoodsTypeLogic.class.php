...
class GoodsTypeLogic extends BaseLogic{
    ...
    function getList($request = array()) {
        ...
        //设置初始数据
        api('Tree/init',array($tree_data,$template,array('id','cn_type_name','ue_type_name','is_app_show','classify_show','sort','operate')));
        //生成树状页面
    ...
    function handleOperate($list = array(), $model = '', $prohibit = array(), $only_allow = array('add_child','edit','set_status','delete')) {
        ...
        foreach($list as $key => $value) {
            ...
            if( $value['type'] != 3){
                ...
                $add_child = '';
                $calssifyPic = "<a href='".U(''.MODULE_NAME.'/'.$model.'/classifyPic/id/'.$value['id'])."' title='分类图片' class='tip-bottom'>
                            <span class='label ' style='background:#543E1F;'>分类图片</span></a>";
                if($value['classify_show'] == 0) {
                    //启用html
                    $classify_show = "<a href='".U(''.MODULE_NAME.'/'.$model.'/hotResume/model/'.$model.'/ids/'.$value['id'].'/classify_show/1')."' title='热门市场' class='tip-bottom ajax-get'>
                                <span class='label label-info'>上热门</span></a>&nbsp;";
                } elseif($value['classify_show'] == 1) {
                    //禁用html
                    $classify_show = "<a href='".U(''.MODULE_NAME.'/'.$model.'/hotResume/model/'.$model.'/ids/'.$value['id'].'/classify_show/0')."' title='热门市场' class='tip-bottom ajax-get'>
                                <span class='label label-inverse'>下热门</span></a>&nbsp";
                }
            }
            ...
            if($value['classify_show'] == 0){
                $list[$key]['classify_show'] = '不显示';
            }else if($value['classify_show'] == 1){
                $list[$key]['classify_show'] = '显示';
            }
            ...
            if($prohibit && in_array($value['id'],$prohibit) && $only_allow) {
                ...
                if($value['type'] != 3){
                    ...
                    $html = $add_virtue . $edit . $set_status . $classify_show . $delete.$calssifyPic;
                }
            }
     ...
     * 修改状态
     */
    function setHotStatus($request = array())
    {
        //判断参数
        if (empty($request['model']) || empty($request['ids']) || !isset($request['classify_show'])) {
            $this->setLogicError('参数错误！');
            return false;
        }
        //执行前操作
        ...
        $data = array(
            'classify_show' => $request['classify_show'],
            'update_time' => time()
        );
    ...
    function remove($request = array()) {
        $countModel = M('goods_type') -> where(array('parent_id'=>$_REQUEST['ids']))->field('id')->count();
        if($countModel > 0 ){
            $this->setLogicError('请先删除该分类的子属性！'); return false;
        }
        $countValueModel = M('attribute')->where(array('thr_g_t_id'=>$request['ids']))->field('id')->count();
        if($countValueModel > 0){
            $this->setLogicError('请先删除该分类下的商品属性！'); return false;
