...
class OrderModel extends BaseModel {
    ...
    function getList($param = array()) {
        ...
        $awaitOrder = M('order')->where(array('status'=>1))->count(); // 等待接单数量
        $deliverOrder = M('order')->where(array('status'=>2))->count(); // 待发货数量
        $list = $model->select();
        foreach($list as $k=>$v){
            $list[$k]['m_id'] = M('Member') ->where(array('id'=>$v['m_id']))->getField('account');
            $list[$k]['address'] = unserialize($v['address']);
        }
        return array('deliverOrder'=>$deliverOrder,'awaitOrder'=>$awaitOrder,'list'=>$list,'page'=>!empty($page_show) ? $page_show : '');
    }
